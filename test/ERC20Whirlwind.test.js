...
const Whirlwind = artifacts.require('./ERC20Whirlwind.sol')
const BadRecipient = artifacts.require('./BadRecipient.sol')
const Token = artifacts.require('./ERC20Mock.sol')
const USDTToken = artifacts.require('./IUSDT.sol')
const { NATIVE_AMOUNT, TOKEN_AMOUNT, MERKLE_TREE_HEIGHT, ERC20_TOKEN } = process.env
...
contract('ERC20Whirlwind', accounts => {
  let whirlwind
  let token
  let usdtToken
  let badRecipient
  ...
  let tokenDenomination = TOKEN_AMOUNT || '1000000000000000000' // 1 native asset
  let snapshotId
  let prefix = 'test'
  let tree
  const fee = bigInt(NATIVE_AMOUNT).shr(1) || bigInt(1e17)
  const refund = NATIVE_AMOUNT || '1000000000000000000' // 1 native asset
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const tokenFromContract = await whirlwind.token()
  ...
  describe('#deposit', () => {
    it('should work', async () => {
      ...
      await token.approve(whirlwind.address, tokenDenomination)

      let { logs } = await whirlwind.deposit(commitment, { from: sender })
    ...
    it('should not allow to send the native asset on deposit', async () => {
      const commitment = toFixedHex(43)
      await token.approve(whirlwind.address, tokenDenomination)

      let error = await whirlwind.deposit(commitment, { from: sender, value: 1e6 }).should.be.rejected
      error.reason.should.be.equal('native asset value is supposed to be 0 for ERC20 instance')
    })
  })
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      await token.approve(whirlwind.address, tokenDenomination, { from: user })
      // Uncomment to measure gas usage
      // let gas = await whirlwind.deposit.estimateGas(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await whirlwind.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      ...
      const balanceWhirlwindBefore = await token.balanceOf(whirlwind.address)
      ...
      // Uncomment to measure gas usage
      // gas = await whirlwind.withdraw.estimateGas(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })
      // console.log('withdraw gas:', gas)
      const { logs } = await whirlwind.withdraw(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })

      const balanceWhirlwindAfter = await token.balanceOf(whirlwind.address)
      ...
      balanceWhirlwindAfter.should.be.eq.BN(toBN(balanceWhirlwindBefore).sub(toBN(tokenDenomination)))
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await whirlwind.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' }).should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')


      ;({ reason } = await whirlwind.withdraw(proof, ...args, { value: toBN(refund).mul(toBN(2)), from: relayer, gasPrice: '0' }).should.be.rejected)
      reason.should.be.equal('Incorrect refund amount received by the contract')
    })
    ...
    it.skip('should work with REAL USDT', async () => {
      ...
      await usdtToken.approve(whirlwind.address, tokenDenomination, { from: user })
      console.log('approve done')
      const allowanceUser = await usdtToken.allowance(user, whirlwind.address)
      ...
      const balanceWhirlwindBefore = await usdtToken.balanceOf(whirlwind.address)
      ...
      const ethBalanceRecieverBefore = await web3.eth.getBalance(toFixedHex(recipient, 20))
      let isSpent = await whirlwind.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(false)
      ...
      const balanceWhirlwindAfter = await usdtToken.balanceOf(whirlwind.address)
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN)
      isSpent = await whirlwind.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(true)
    })
