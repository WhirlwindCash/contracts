...
function snarkVerify(proof) {
  ...
  const verification_key = unstringifyBigInts2(require('../circuits/withdraw_verification_key.json'))
  return snarkjs['groth'].isValid(verification_key, proof, proof.publicSignals)
}

contract('NativeWhirlwind', accounts => {
  let whirlwind
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const bnbDenomination = await whirlwind.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await whirlwind.deposit(commitment, { value, from: sender })
      ...
      ({ logs } = await whirlwind.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await whirlwind.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await whirlwind.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await whirlwind.deposit.estimateGas(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await whirlwind.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceWhirlwindBefore = await web3.eth.getBalance(whirlwind.address)
      ...
      let isSpent = await whirlwind.isSpent(toFixedHex(input.nullifierHash))
      ...
      // Uncomment to measure gas usage
      // gas = await whirlwind.withdraw.estimateGas(proof, ...args, { from: relayer, gasPrice: '0' })
      // console.log('withdraw gas:', gas)
      const { logs } = await whirlwind.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceWhirlwindAfter = await web3.eth.getBalance(whirlwind.address)
      ...
      balanceWhirlwindAfter.should.be.eq.BN(toBN(balanceWhirlwindBefore).sub(toBN(value)))
      ...
      isSpent = await whirlwind.isSpent(toFixedHex(input.nullifierHash))
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      await tree.insert(deposit.commitment)
      await whirlwind.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await whirlwind.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await whirlwind.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await whirlwind.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await whirlwind.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await whirlwind.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await whirlwind.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
    ...
    it('should reject with non zero refund', async () => {
      ...
      const error = await whirlwind.withdraw(proof, ...args, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Refund value is supposed to be zero for native asset instance')
    })
  })

  describe('#changeOperator', () => {
    it('should work', async () => {
      let operator = await whirlwind.operator()
      operator.should.be.equal(sender)

      const newOperator = accounts[7]
      await whirlwind.changeOperator(newOperator).should.be.fulfilled

      operator = await whirlwind.operator()
    ...
    it('cannot change from different address', async () => {
      ...
      const newOperator = accounts[7]
      const error = await whirlwind.changeOperator(newOperator, { from:  accounts[7] }).should.be.rejected
  ...
  describe('#updateVerifier', () => {
    it('should work', async () => {
      ...
      await whirlwind.updateVerifier(newVerifier).should.be.fulfilled

      const verifier = await whirlwind.verifier()
    ...
    it('cannot change from different address', async () => {
      ...
      const newVerifier = accounts[7]
      const error = await whirlwind.updateVerifier(newVerifier, { from:  accounts[7] }).should.be.rejected
      error.reason.should.be.equal('Only operator can call this function.')
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await whirlwind.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await whirlwind.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await whirlwind.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await whirlwind.isSpentArray([nullifierHash1, nullifierHash2])
